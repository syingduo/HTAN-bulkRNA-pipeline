...
def readcount_to_fpkm(gene_info_df, rc_df):
    ...
    with np.errstate(divide='ignore'):
        ...
        tpm = np.exp(np.log(fpkm) - np.log(np.sum(fpkm)) + np.log(10**6))

    gene_basic_info_df = gene_info_df.loc[:, ['gene_name', 'hgnc_id']]
    fpkm_df = pd.concat(
        [gene_basic_info_df, rc_per_gene, fpkm, fpkm_uq, tpm], 
        axis=1, join='inner', sort=False
    )
    fpkm_df.columns = ['symbol', 'hgnc_id', 'read_count', 'fpkm', 'fpkm_uq', 'tpm']
